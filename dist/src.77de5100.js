...
var ball = 2;
var brick = 3;
var createElement = function createElement(col) {
  ...
  div.style.width = "6px";
  div.style.backgroundColor = col === empty ? "white" : col === player ? "cornflowerblue" : col === ball ? "gray" : "silver";
  div.style.borderRadius = col === ball ? "100%" : "0%";
  return div;
};
var paint = function paint(_a) {
  var player = _a[0],
    ball = _a[1],
    bricks = _a[2];
  document.body.innerHTML = "Lives: ".concat(player.lives, ", Score: ").concat(player.score);
  var game = Array(constants_1.gameSize).fill(0).map(function (e) {
    return Array(constants_1.gameSize).fill(0);
  });
  game[player.x][player.y] = player;
  game[ball.x][ball.y] = ball;
  bricks.forEach(function (b) {
    return game[b.x][b.y] = brick;
  });
  game.forEach(function (row) {
    var rowContainer = document.createElement("div");
    row.forEach(function (col) {
      return rowContainer.appendChild(createElement(col));
    });
...
var createGameObject = function createGameObject(x, y) {
  return {
    x: x,
    y: y
  };
};
var player$ = (0, rxjs_1.combineLatest)((0, rxjs_1.of)(__assign(__assign({}, createGameObject(constants_1.gameSize - 2, constants_1.gameSize / 2 - 1)), {
  score: 0,
  lives: 3
})), (0, rxjs_1.fromEvent)(document, "keyup").pipe((0, rxjs_1.startWith)({
  code: ""
}), (0, rxjs_1.map)(function (e) {
  return e.code;
}))).pipe((0, rxjs_1.map)(function (_a) {
  var player = _a[0],
    key = _a[1];
  return key === "ArrowLeft" ? player.y -= 1 : key === "ArrowRight" ? player.y += 1 : rxjs_1.noop, player;
}));
var ball$ = (0, rxjs_1.combineLatest)((0, rxjs_1.of)(__assign(__assign({}, createGameObject(constants_1.gameSize / 2, constants_1.gameSize - 3)), {
  dirX: 1,
  dirY: 1
})), (0, rxjs_1.interval)(150)).pipe((0, rxjs_1.map)(function (_a) {
  var ball = _a[0],
    _ = _a[1];
  return ball.dirX *= ball.x > 0 ? 1 : -1, ball.dirY *= ball.y > 0 && ball.y < constants_1.gameSize - 1 ? 1 : -1, ball.x += 1 * ball.dirX, ball.y -= 1 * ball.dirY, ball;
}));
var bricks$ = (0, rxjs_1.generate)(1, function (x) {
  return x < 8;
}, function (x) {
  return x + 1;
}).pipe((0, rxjs_1.mergeMap)(function (r) {
  return (0, rxjs_1.generate)(r % 2 === 0 ? 1 : 0, function (x) {
    return x < constants_1.gameSize;
  }, function (x) {
    return x + 2;
  }).pipe((0, rxjs_1.map)(function (c) {
    return createGameObject(r, c);
  }));
}), (0, rxjs_1.toArray)());
var processGameCollisions = function processGameCollisions(_, _a) {
  var player = _a[0],
    ball = _a[1],
    bricks = _a[2];
  return function (collidingBrickIndex) {
    return collidingBrickIndex > -1 ? (bricks.splice(collidingBrickIndex, 1), ball.dirX *= -1, player.score++) : rxjs_1.noop;
  }(bricks.findIndex(function (e) {
    return e.x === ball.x && e.y === ball.y;
  })), ball.dirX *= player.x === ball.x && player.y === ball.y ? -1 : 1, ball.x > player.x ? (player.lives--, ball.x = constants_1.gameSize / 2 - 3) : rxjs_1.noop, [player, ball, bricks];
};
(0, rxjs_1.combineLatest)(player$, ball$, bricks$).pipe((0, rxjs_1.scan)(processGameCollisions), (0, rxjs_1.tap)(html_renderer_1.paint), (0, rxjs_1.takeWhile)(function (_a) {
  var player = _a[0];
  return player.lives > 0;
})).subscribe();
},{"rxjs":"../node_modules/rxjs/dist/esm5/index.js","./constants":"constants.ts","./html-renderer":"html-renderer.ts"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
